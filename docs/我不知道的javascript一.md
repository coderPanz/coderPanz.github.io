...
**缺点**。
首先，你可能会认为 ES6 的 class 语法是向 JavaScript 中引入了一种新的“类”机制，其实不是这样。class 基本上只是现有 [[Prototype]]（委托！）机制的一种语法糖。class 语法无法定义类成员属性（只能定义方法， java这种的可以定义成员变量）。
...
为什么 js 中的 class 语法不能定义类成员属性？  
因为 class 也是基于原型链的，如果定义了成员属性，那么所有实例都会共享这个属性，这样当一个实例修改了成员属性，其他实例也会受到影响，所以设计者没有让 class 语法定义成员属性。

#### 小结
class 很好地伪装成 JavaScript 中类和继承设计模式的解决方案。
